      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full overflow-hidden border border-slate-200">
        ...
        <div className="px-6 py-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
          <div>
            ...
            <p className="text-xs text-slate-500 font-mono">Reference: {identifier}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
          ...
          {loading ? (
            ...
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 bg-slate-50/80 p-3 rounded-lg border border-slate-200 text-xs">
                <div>
                  <span className="text-slate-400 text-[11px] block">Reference</span>
                  <span className="font-mono font-bold text-slate-800">{transaction.referenceId}</span>
                </div>
                <div>
                  <span className="text-slate-400 text-[11px] block">Posted At (UTC)</span>
                  <span className="font-mono text-slate-700">{new Date(transaction.postedAtUtc).toLocaleString()}</span>
                </div>
                <div className="col-span-2 sm:col-span-1">
                  <span className="text-slate-400 text-[11px] block">Description</span>
                  <span className="text-slate-800 font-medium">{transaction.description}</span>
                </div>
              </div>
                    ...
                    {transaction.splits.map((s, idx) => (
                      <tr key={idx}>
                        <td className="py-2 px-3">
                          <span className="font-mono font-bold">{s.accountNumber}</span> - {s.accountName}
                        </td>
                        <td className="py-2 px-3">
                          <span
